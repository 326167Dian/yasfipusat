        ...
        <tr>
            ...
            <td style='text-align:center;'>
                <?php 
                    ...
                    $batch  = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM batch WHERE kd_barang = '$d[kd_barang]'                                                                            
                                                                            AND exp_date > '$datenow'");
                    
                    while($bt     = mysqli_fetch_array($batch)){
                        $detail = $db->query("SELECT sum(qty) as msk FROM batch WHERE no_batch = '$bt[no_batch]' and status = 'masuk'")->fetch_array();
                        $ttlmasuk = $detail['msk'];
                        $detail2 = $db->query("SELECT sum(qty) as klr FROM batch WHERE no_batch = '$bt[no_batch]' and status = 'keluar'")->fetch_array();
                        $ttlkeluar = $detail2['klr'];
                        $sisa = $ttlmasuk - $ttlkeluar;
                        if($sisa > 0 and $bt['no_batch'] !=''){
                            echo $bt['no_batch'] .  ' ('. $bt['exp_date'] .' )'.' ('. $sisa .' )' ."<br>";
                        }
