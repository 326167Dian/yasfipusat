...
$pdf->Image('../../images/'.$rh['logo'],3,1.45,2,2.5,'');
$pdf->ln(1);
...
$pdf-> SetXY($current_x,$current_y);
$pdf->MultiCell(1, 1.2, 'Qty', 1, 'C');
$current_x = (1 + $current_x);
$pdf-> SetXY($current_x,$current_y);
$pdf->MultiCell(2.4, 1.2, 'Keterangan', 1, 'C');


$no = 1;
$query1 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT *
FROM ordersdetail left join barang
on (ordersdetail.id_barang=barang.id_barang)
...
while ($lihat = mysqli_fetch_array($query1)) {
    ...
    $ket = terbilang($qty);
    $text13 = substr($ket,0,15);
    $text14 = substr($ket,15,28);
    $text15 = strlen($ket);
    ...
    if($text6<47 || $text9<26) {
        ...
        $pdf->Cell(1, 0.7, $qty, 1, 0, 'C');
        $pdf->Cell(2.4, 0.7, $text13, 1, 1, 'C');
    }
    else {
        $pdf->SetFont('Arial', '', 8);
        ...
        $pdf->Cell(1, 0.7, $qty, 'LTR', 0, 'C');
        $pdf->Cell(2.4, 0.7, $text13, 'LTR', 1, 'C');
...
if($res['ttd_digital']=='YA'){
$pdf->Image('../../images/'.$rh['tandatangan'],14.5,$y,3.5,3.5,'');}

if($res['stempel']=='YA'){
$pdf->Image('../../images/'.$rh['stempel'],12,$y,3.4,3.4,'');}
