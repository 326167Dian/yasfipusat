...
elseif ($module=='trkasir' AND $act=='hapus'){
	...
	try {
		...
		$ambildatadetail = run_query_or_fail_aksi_shift($conn, "SELECT id_dtrkasir, kd_trkasir, id_barang, kd_barang, no_batch, qty_dtrkasir FROM dropping_detail WHERE kd_trkasir='$kd_trkasir'");
		while ($r = mysqli_fetch_array($ambildatadetail)) {
			...
			run_query_or_fail_aksi_shift($conn, "DELETE FROM dropping_detail WHERE id_dtrkasir = '$id_dtrkasir'");
