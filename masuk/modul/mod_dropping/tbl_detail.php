			...
			<div class="box-body table-responsive">
					<table id="example5" class="table table-condensed table-bordered table-striped table-hover" >
    					...
    					$aksi_trkasir = "masuk/modul/mod_dropping/aksi_dropping.php";
							...
							$dfoot=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM dropping 
							join carabayar on dropping.id_carabayar = carabayar.id_carabayar
							...
							$sumprice=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT kd_trkasir, SUM(hrgttl_dtrkasir) as grandnya FROM dropping_detail 
							WHERE kd_trkasir='$kd_trkasir'");
							$ttlprice=mysqli_fetch_array($sumprice);
							$grandnya = format_rupiah($ttlprice['grandnya']);
						
						       $noreq=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM dropping_detail
							   WHERE dropping_detail.kd_trkasir='$kd_trkasir'
							   ORDER BY dropping_detail.id_dtrkasir ASC");
								$no=1;
						...
						<legend class='scheduler-border'></legend>
							<div class='col-md-6'>	
											...
											<div class='buttons'>
												...
												<a href=javascript:confirmdelete('$aksi?module=dropping&act=hapus&id=$rf[id_trkasir]') title='HAPUS' class='btn btn-danger right-block'>HAPUS</a> 
											</div>
											";
											}
											echo"
