...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
 ...
 <center>Untuk mengakses modul, Anda harus login <br>";
	...
	function sync_dropping_to_yasfi2($connSource, $connTarget, $kdTrdropping, $kdTrkasir)
	{
		...
		try {
			$headerResult = run_query_or_fail_aksi_trkasir($connSource, "SELECT * FROM trkasir WHERE kd_trkasir = '$kdTrkasirSource' LIMIT 1");
			$header = mysqli_fetch_assoc($headerResult);
			if (!$header) {
				throw new Exception('Data trkasir untuk dropping tidak ditemukan');
			}
        ...
        if($ubah){
            try {
                if ($_POST['id_carabayar'] == 4 and $_POST['trdroping'] == 'ubah') {
                    $trkasir = run_query_or_fail_aksi_trkasir($conn, "SELECT kd_trkasir FROM trkasir WHERE id_trkasir = '$_POST[id_trkasir]' LIMIT 1");
	...
	elseif ($module == 'trkasir' and $act == 'hapus') {
		...
		if ($_SESSION['level'] != 'pemilik') {
			...
			try {
				...
				$ambildatainduk = run_query_or_fail_aksi_trkasir($conn, "SELECT * FROM trkasir WHERE id_trkasir='$id_trkasir'");
				$r1 = mysqli_fetch_array($ambildatainduk);
				if (!$r1) {
				...
				$kd_trkasir = $r1['kd_trkasir'];
				$ambildatadetail = run_query_or_fail_aksi_trkasir($conn, "SELECT * FROM trkasir_detail WHERE kd_trkasir='$kd_trkasir'");
				while ($r = mysqli_fetch_array($ambildatadetail)) {
					...
					run_query_or_fail_aksi_trkasir($conn, "DELETE FROM trkasir_detail WHERE id_dtrkasir = '$id_dtrkasir'");
				...
				run_query_or_fail_aksi_trkasir($conn, "DELETE FROM trkasir WHERE id_trkasir = '$id_trkasir'");
